...
<div class="w-1/2 mx-auto text-center py-4 mt-8 font-poppins bg-white">
    <h2 class="pt-4 text-2xl font-bold mb-4">Upload Announcement</h2>
...
<div class="text-center w-1/2 mx-auto mt-8 font-poppins bg-white px-4">
    ...
    <div class="mb-4 grid items-center rounded-lg"  style="background-color: {{ $data->role === 'user' ? '#1A5C34' : 'transparent' }};">
    <hr>
    <p class="text-gray-600 font-medium mt-6" style="color: {{ $data->role === 'user' ? 'white' : 'black' }};">{{ $data->forum_content }}</p>
        <p class="text-gray-600 font-bold " style="color: {{ $data->role === 'user' ? 'white' : 'black' }};">{{ $data->user_name }}</p>
        <p class="text-gray-600 font-bold mb-6" style="color: {{ $data->role === 'user' ? 'white' : 'black' }};">{{ ucfirst($data->role) }}</p>
